    ...
    async def send_renewal_message(self, user_id):
        async with client:
            ...
            if user and user.get('chat_id'):
                await client.send_message(chat_id=user['chat_id'], text="Your plan has expired. Please renew to continue enjoying our services.")

    async def check_and_notify_expired_plans(self):
        now = datetime.datetime.now()
        users = self.col.find({'expiry_time': {'$lte': now}})
